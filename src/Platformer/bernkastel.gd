...
func bernkastel_battle():
	...
	if not is_dead and not miracle.game_root.player.is_dead:
		...
		yield(self, "stopped_moving")
		$Trap.play()
		bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * 3, 64 * -1)}, {"time_before_trigger" : 0})
		move(Vector2(1100, 700))
		yield(self, "stopped_moving")
		$Trap.play()
		bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * 4, 64 * -1)}, {"time_before_trigger" : 0})
		bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * 3, 64 * -1)}, {"time_before_trigger" : 0})
		bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * 2, 64 * -1)}, {"time_before_trigger" : 0})
		bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * 1, 64 * -1)}, {"time_before_trigger" : 0})
		move(Vector2(1100, 700))
		yield(self, "stopped_moving")
		move(Vector2(1800, 400))
		yield(self, "stopped_moving")
	...
	if not is_dead and not miracle.game_root.player.is_dead:
		...
		for i in [12, 9, [7, 6], [8, 12], [9, 11], 10, [8, 12], 11, 10, [6, 9, 10], [7, 12], 8, [9, 11], 10, 11]:
			...
			if typeof(i) == TYPE_INT:
				bulletmanager.spike({"rotation_deg" : 270, "position" : Vector2(64 * 17, 64 * i)}, {"time_before_trigger" : 0})
	...
	if not is_dead and not miracle.game_root.player.is_dead:
		...
		for i in [12, [1, 3, 4], [7, 6], [8, 13, 12, 1, 4, 5], [9, 11, 4, 6, 5, 7, 1, 16, 15], 14, 10, [8, 12, 10, 6, 7, 16], 11, [10, 16], [6, 9, 10], [7, 12], 8, [9, 11], 10, 11]:
			$Trap.play()
			if typeof(i) == TYPE_INT:
				bulletmanager.spike({"rotation_deg" : 180, "position" : Vector2(64 * i, 64 * -1)}, {"time_before_trigger" : 0})
			elif typeof(i) == TYPE_ARRAY:
				for s in i:
